...
const LANDING_BUILD = "20260524-a11y";
const LANDING_PAYLOAD_MARKER = "---LANDING_PAYLOAD---";

const CONFIG = {
  brand: "",
  tagline: "המומחים בסדר הביטוחי שלך",
  phone: "04-6043579",
  ...
  accessibility: {
    company: "א.ס פיננסים",
    phone: "054-234-5622",
    phoneTel: "tel:+972542345622",
    email: "itayso@i-s-f.co.il",
...
function initAccessibilityWidget() {
  ...
  function applyPrefs() {
    ...
    if (textScale === 1) document.body.classList.add("a11y-text-1");
    if (textScale >= 2) document.body.classList.add("a11y-text-2");
  }

  function savePrefs() {
    try {
      localStorage.setItem(A11Y_STORAGE_KEY, JSON.stringify({ textScale, highContrast }));
    } catch (_) {}
  }
  ...
  function loadPrefs() {
    try {
      ...
      textScale = Number(data.textScale) || 0;
  ...
  panel.querySelectorAll("[data-a11y-action]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      if (action === "text-up") textScale = Math.min(2, textScale + 1);
      else if (action === "text-down") textScale = Math.max(0, textScale - 1);
      else if (action === "contrast") highContrast = !highContrast;
...
function initLeadForm() {
  ...
  const personalFieldIds = ["fullName", "idNumber", "phone", "notes"];
  ...
  function setPersonalStepVisible(visible) {
    ...
    personalFieldIds.forEach((id) => {
      ...
      if (visible) {
        ...
        if (id !== "notes") input.setAttribute("aria-required", "true");
      } else {
