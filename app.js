...
const CONFIG = {
  ...
  siteUrl: "https://gemelinvest.github.io",
  brand: "",
  ...
  accessibility: {
    ...
    coordinatorLabel: "מחלקת שירות — א.ס פיננסים"
  },
...
function initLegalContact() {
  ...
  const coordinator = CONFIG.accessibility?.coordinatorLabel || company;
...
function initFooterLegal() {
  ...
  if (companyEl) companyEl.textContent = CONFIG.companyName;
  if (addressEl) addressEl.textContent = CONFIG.address;
  if (copyrightNameEl) copyrightNameEl.textContent = CONFIG.companyName;
