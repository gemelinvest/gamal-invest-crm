 ...
 * BUILD: 20260522-rpc-fix — העלו קובץ זה ל-GitHub (app.js)
 */

const LANDING_BUILD = "20260522-rpc-fix";
...
async function sendLeadToCRM(leadData) {
  ...
  if (!res.ok) {
    ...
    throw new Error(data.message || data.error || data.hint || "http_" + res.status);
  }
      ...
      const msg =
        code === "not_configured"
          ...
          : code === "unauthorized"
            ...
            : code === "invalid_phone"
              ? "מספר טלפון לא תקין"
              : "אירעה שגיאה. נסו שוב או צרו קשר בטלפון.";
