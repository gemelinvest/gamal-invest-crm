...
function initFooterLegal() {
  ...
  if (addressEl) {
    ...
    addressEl.hidden = !safeTrim(CONFIG.address);
  }
  if (copyrightNameEl) {
...
function initReviewsSlider() {
  ...
  track.innerHTML = CONFIG.reviews
    .map(
      (r, i) => `
      <article class="review-card" data-index="${i}" aria-roledescription="slide">
...
function initLeadForm() {
  ...
  const insuranceStep = document.getElementById("leadInsuranceStep");
  const personalStep = document.getElementById("leadPersonalStep");
  const continueBtn = document.getElementById("leadContinue");
  ...
  const personalFieldIds = ["fullName", "phone", "notes", "privacyConsent"];
  ...
  function setInsuranceStepVisible(visible) {
    if (!insuranceStep) return;
    insuranceStep.hidden = !visible;
    insuranceStep.setAttribute("aria-hidden", visible ? "false" : "true");
  }
  ...
  function setPersonalStepVisible(visible) {
    ...
    setInsuranceStepVisible(!visible);
  ...
  function updateContinueButton() {
    ...
    if (!continueBtn || personalStepVisible) return;
    continueBtn.hidden = !hasSelection;
    continueBtn.disabled = !hasSelection;
  }

  function onInsuranceSelectionChange() {
    validateField("insuranceTypes");
    if (!personalStepVisible) updateContinueButton();
  }

  function goToPersonalStep() {
    if (!validateField("insuranceTypes")) return;
  ...
  continueBtn?.addEventListener("click", goToPersonalStep);

  Object.keys(fields).forEach((id) => {
    if (id === "insuranceTypes") {
      typesList.addEventListener("change", onInsuranceSelectionChange);
      return;
    }
